...
# Función personalizada para ordenar los datos
def ordenar_datos(datos):
    for i in range(len(datos)):
        for j in range(i + 1, len(datos)):
            if (datos[j]["Ptos"] > datos[i]["Ptos"]) or (
                datos[j]["Ptos"] == datos[i]["Ptos"] and datos[j]["DG"] > datos[i]["DG"]
            ):
                # Intercambiar
                datos[i], datos[j] = datos[j], datos[i]

# Ordenar los datos utilizando la función personalizada
ordenar_datos(datos)
